...
class KeyPressListener {
    ...
    constructor(keyCodes, callback) {
        ...
        this.keydownFunction = function (event) {
            if (verificaKey(event.code)) { // Verifica se o código da tecla pressionada corresponde ao keyCode fornecido
        ...
        this.keyupFunction = function (event) {
            if (verificaKey(event.code)) { // Verifica se o código da tecla solta corresponde ao keyCode fornecido
                keySafe = true; // Marca a tecla como segura para ser pressionada novamente
            }
        };

        const verificaKey = (code) => { return keyCodes.includes(code); };
